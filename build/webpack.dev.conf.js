...
module.exports = new Promise((resolve, reject) => {
  ...
  portFinder.getPortPromise()
    .then((port) => {
      ...
      const localUrl = process.env.PROXY ? `http://xxx.xxx.com:${port}${config.dev.baseUrl}/` : `http://localhost:${port}${config.dev.baseUrl}/`
      const networkUrl = `http://${address.ip()}:${port}${config.dev.baseUrl}/`
      const messages = process.env.PROXY ? [
  `App running at:
  
    - Local: ${chalk.cyan(localUrl)}
    - Network: ${chalk.cyan(networkUrl)}

    请配置Host: 127.0.0.1 xxx.xxx.com
